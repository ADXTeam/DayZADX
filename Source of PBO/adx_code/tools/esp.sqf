...
if (isnil "mapm") then 
{
	if (mautoaddmap and !("ADX_m" in items player)) then {player addweapon "ADX_m";};			
	...
	if musekeys then 
	{
		F2_EH = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) == 0x3D) then {call moptions;};"];
	};

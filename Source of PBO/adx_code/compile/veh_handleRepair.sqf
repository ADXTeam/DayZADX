...
if (local _unit) then {
    if (_selection in ["pluhPredni","kolaOchrana","oknaOchrana","predniOknoOchrana"]) then {
        _unit setVariable [_selection,0,true];
        _unit animate [_selection,0];
        _unit setHit [_selection, 0];
        diag_log format ["Upgrade = _selection =%1,_damage=%2",_selection,0];
        //PVDZ_veh_SF = [_unit,"repair"];
    } else {

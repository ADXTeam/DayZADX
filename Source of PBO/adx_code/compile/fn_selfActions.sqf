...
************************************************************/
private ["_hastinitem","_canPickLight","_text","_driver","_hasRawMeat","_isFuel","_menu","_isHarvested","_isVehicle","_isVehicletype","_isMan","_ownerID","_isAnimal","_isZombie","_isDestructable","_isTent","_isStash","_isMediumStash","_hasFuel80","_hasFuel20","_hasFuel5","_isAlive","_canmove","_isPlant","_rawmeat","_vehicle","_inVehicle","_hasFuelE20","_hasFuelE80","_hasFuelE5","_cursorTarget","_hasbottleitem","_primaryWeapon","_currentWeapon","_hasKnife","_hasToolbox","_onLadder","_nearLight","_canDo","_maxbbLevel","_levelhouse","_naObnovku","_nextlvl"];


//titletext ["NEW SF", "PLAIN DOWN"];
...
_hasFuelE80 = "ItemJerrycanBEmpty" in magazines player;
...
_newTypeB = ["Uroven1DrevenaBudka","Uroven2KladaDomek","Uroven3DrevenyDomek","Uroven1VelkaBudka","Uroven2MalyDomek","Uroven3VelkyDomek","malaGaraz","velkaGaraz","object_x","krepost"];
...
if (!isNull cursorTarget and !_inVehicle and (player distance cursorTarget < 30)) then {
    ...
    //if(_ownerID == _playerID) then {"krepost"
    if(_newCuTyp in _newTypeB) then {
        if ((_newCuTyp == "Uroven1DrevenaBudka") 	OR (_newCuTyp == "Uroven1VelkaBudka")) 	then { _maxbbLevel = 2; };
        if ((_newCuTyp == "Uroven2KladaDomek") 	OR (_newCuTyp == "Uroven2MalyDomek")) 	then { _maxbbLevel = 4; };
        if ((_newCuTyp == "Uroven3DrevenyDomek") OR (_newCuTyp == "Uroven3VelkyDomek")) 	then { _maxbbLevel = 8; };
        if ((_newCuTyp == "malaGaraz") OR (_newCuTyp == "velkaGaraz")) 	then { _maxbbLevel = 4; };
        if (_newCuTyp == "object_x") 	then { _maxbbLevel = 1; };
        if (_newCuTyp == "krepost") 	then { _maxbbLevel = 23; };
        ...
        if (_levelhouse < _maxbbLevel) then {
            if ((s_player_1bupd < 0) and (player distance cursorTarget > 8) and (player distance cursorTarget < 30)) then {
                s_player_1bupd = player addAction [format[localize "STR_BUILDING_STAGE_2",_nextlvl], "\z\addons\adx_code\actions\bse_pp.sqf",[cursorTarget,_levelhouse], 0, false, true, "",""];
            };
        };
        if ((_levelhouse == 1)&&(_newCuTyp != "object_x") ) then {
            if ((s_player_packFdp < 0) and (player distance cursorTarget > 8) and (player distance cursorTarget < 30)) then {
                s_player_packFdp = player addAction [localize "STR_BUILDING_STAGE_U", "\z\addons\adx_code\actions\player_dFp.sqf",cursorTarget, 0, false, true, "",""];
            };
        };
        if (_newCuTyp == "krepost") then {
            if (_levelhouse > 8) then {
                if ((s_player_otkdv < 0) and (player distance cursorTarget < 30)) then {
                    diag_log format ["ADD ACTION DOM _naObnovku=%1",_naObnovku];
                    if(!_naObnovku) then {
                        s_player_otkdv = player addAction ["Un-lock house, open doors", "\z\addons\adx_code\actions\player_ppver.sqf",[cursorTarget,1], 0, false, true, "",""];
                    };
                    if(_naObnovku) then {
                        s_player_otkdv = player addAction ["Lock house, close doors", "\z\addons\adx_code\actions\player_ppver.sqf",[cursorTarget,2], 0, false, true, "",""];
                    };
                };
            };
        };
        
        if ((_newCuTyp == "Uroven1DrevenaBudka") OR (_newCuTyp == "Uroven1VelkaBudka") OR (_newCuTyp == "Uroven3VelkyDomek")) then {
        ...
        if ((_newCuTyp == "Uroven2KladaDomek") OR (_newCuTyp == "Uroven2MalyDomek") OR (_newCuTyp == "Uroven3DrevenyDomek") OR (_newCuTyp == "malaGaraz") OR (_newCuTyp == "velkaGaraz")) then {
    ...
    _hasFuel80 = "ItemJerrycanB" in magazines player;
    ...
    if (_hasFuelE20 or _hasFuelE5 or _hasFuelE80) then {
    ...
    if((_hasFuelE20 or _hasFuelE5 or _hasFuelE80) and _isFuel and _canDo and !a_player_jerryfilling) then {
        if (s_player_fillfuel < 0) then {
            s_player_fillfuel = player addAction [localize "str_actions_self_10", "\z\addons\adx_code\actions\jerry_fill.sqf",[], 1, false, true, "", ""];
        };
    } else {
    ...
    if(_hasFuel20 and _canDo and _isVehicle and (fuel _cursorTarget < 1)) then {
        if (s_player_fillfuel20 < 0) then {
            s_player_fillfuel20 = player addAction [format[localize "str_actions_medical_10",_text,"20"], "\z\addons\adx_code\actions\refuel.sqf",["ItemJerrycan"], 0, true, true, "", "'ItemJerrycan' in magazines player"];
        };
    } else {
    ...
    if(_hasFuel5 and _canDo and _isVehicle and (fuel _cursorTarget < 1)) then {
        if (s_player_fillfuel5 < 0) then {
            s_player_fillfuel5 = player addAction [format[localize "str_actions_medical_10",_text,"5"], "\z\addons\adx_code\actions\refuel.sqf",["ItemFuelcan"], 0, true, true, "", "'ItemFuelcan' in magazines player"];
        };
    } else {
    ...
    if(_hasFuel80 and _canDo and _isVehicle and (fuel _cursorTarget < 1)) then {
        if (s_player_fillfuel80 < 0) then {
            s_player_fillfuel80 = player addAction [format[localize "str_actions_medical_10",_text,"80"], "\z\addons\adx_code\actions\refuel.sqf",["ItemJerrycanB"], 0, true, true, "", "'ItemJerrycanB' in magazines player"];
        };
    } else {
    ...
    if ((_hasFuelE20 or _hasFuelE5 or _hasFuelE80) and _isVehicle and _canDo and !a_player_jerryfilling and (fuel _cursorTarget > 0)) then {
    ...
    _hasClay = "ItemClay" in magazines player;
    if (inflamed _cursorTarget and _hasClay  and _canDo and !a_player_brick) then {
        if (s_player_brick < 0) then {
            s_player_brick = player addAction ["Сделать кирпичи", "\z\addons\adx_code\actions\makeBrick.sqf",_cursorTarget, 3, true, true, "", ""];            
        };
        if (s_player_cinder < 0) then {
            s_player_cinder = player addAction ["Сделать шлакоблоки", "\z\addons\adx_code\actions\makeCinder.sqf",_cursorTarget, 3, true, true, "", ""];
    ...
    if ((dayz_myCursorTarget != _cursorTarget) and _isVehicle and !_isMan and _hasToolbox and (damage _cursorTarget < 1)) then {
        if (s_player_repair_crtl < 0) then {
            ...
            _menu = dayz_myCursorTarget addAction [localize "str_actions_rapairveh", "\z\addons\adx_code\actions\repair_vehicle.sqf",_cursorTarget, 0, true, false, "",""];

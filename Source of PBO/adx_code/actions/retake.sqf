...
if (_hasToolbox && (_part != "PartGlass")) then {
    ...
    if (_damage < 0.10) then {
        ...
        if (_findspace) then {
			...
			[_vehicle, _selection, _damage, true] call fnc_veh_handleDam;
            ...
            //Success!
            cutText [format["Вы успешно сняли %1 с %2",_namePart,_nameType], "PLAIN DOWN"];
            _magazines	= call player_countmagazines;
            dayz_lastSave = time;
            dayz_Magazines = [];
        } else {
            cutText ["У вас недостаточно места в инвентаре", "PLAIN DOWN"];
        };
    } else {
        cutText [format["Невозможно снять %1 с %2, часть сильно повреждена",_namePart,_nameType], "PLAIN DOWN"];
    };
